        ...
        <el-row class="inline-info">
          <el-col :span="12">
            <el-form-item label="员工头像">
              <!-- 放置上传图片 -->
              <ImageUpload ref="staffPhoto" :limit="1" />
            </el-form-item>
          </el-col>
        </el-row>
        <!-- 保存个人信息 -->
  ...
  methods: {
    ...
    async getUserDetailById () {
      ...
      if (this.userInfo.staffPhoto) {
        // 这里我们ref赋值，同时需要给赋值的地址一个标记 upload: true
    ...
    async getPersonalDetail () {
      ...
      if (this.formData.staffPhoto) {
        this.$refs.myStaffPhoto.fileList = this.formData.staffPhoto.split(',').map(url => ({ url }))
      }
    },
    async savePersonal () {
      // 保存时读取头像
      ...
      await updatePersonal({ ...this.formData, staffPhoto: fileList.map(item => item.url).join(',') })
